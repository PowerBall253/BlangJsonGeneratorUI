    ...
    public MainWindowViewModel()
    {
        ...
        OpenBlangCommand = ReactiveCommand.CreateFromTask(async () =>
        {
            if (UnsavedChanges && AnyModified)
            {
                ...
                var confirm = await Views.MessageBox.ShowAsync((Application.Current!.ApplicationLifetime as ClassicDesktopStyleApplicationLifetime)!.MainWindow!, "Warning", "Are you sure you want to open another file?\nAll unsaved changes will be lost.", Views.MessageBox.MessageButtons.YesCancel);
            ...
            // Get filepath
            string filePath = await OpenFileDialogAsync("Select the .blang file to load", new List<string> { "blang" }, "Blang files");
            ...
            {
                await Views.MessageBox.ShowAsync((Application.Current!.ApplicationLifetime as ClassicDesktopStyleApplicationLifetime)!.MainWindow!, "Error", "Failed to read from the blang file.\nMake sure the file exists and isn't being used by another process.", Views.MessageBox.MessageButtons.Ok);
                return;
            }
            ...
            {
                await Views.MessageBox.ShowAsync((Application.Current!.ApplicationLifetime as ClassicDesktopStyleApplicationLifetime)!.MainWindow!, "Error", "Failed to load the blang file.\nMake sure the file is valid, then try again.", Views.MessageBox.MessageButtons.Ok);
                return;
            }
        });
        ...
        OpenResourcesCommand = ReactiveCommand.CreateFromTask(async () =>
        {
            ...
            // Get filepath
            string filePath = await OpenFileDialogAsync("Select the .resources file to load", new List<string> { "resources", "backup" }, "Resource files");
            ...
            if (blangFiles == null)
            {
                await Views.MessageBox.ShowAsync((Application.Current!.ApplicationLifetime as ClassicDesktopStyleApplicationLifetime)!.MainWindow!, "Error", "Failed to load the .resources file.\nMake sure the file is valid, then try again.", Views.MessageBox.MessageButtons.Ok);
                return;
            }

            if (blangFiles.Count == 0)
            {
                await Views.MessageBox.ShowAsync((Application.Current!.ApplicationLifetime as ClassicDesktopStyleApplicationLifetime)!.MainWindow!, "Error", "No blang files were found in the .resources file.\nMake sure you chose the right file, then try again.", Views.MessageBox.MessageButtons.Ok);
                return;
            }

            // Let user select the blang to load
            string? selectedBlang = await Views.BlangSelection.ShowAsync((Application.Current!.ApplicationLifetime as ClassicDesktopStyleApplicationLifetime)!.MainWindow!, blangFiles.Keys.ToArray());
        ...
        LoadJsonCommand = ReactiveCommand.CreateFromTask(async () =>
        {
            ...
            {
                // Confirmation message box
                var confirm = await Views.MessageBox.ShowAsync((Application.Current!.ApplicationLifetime as ClassicDesktopStyleApplicationLifetime)!.MainWindow!, "Warning", "Are you sure you want to load a JSON?\nSome unsaved changes may be lost.", Views.MessageBox.MessageButtons.YesCancel);
            ...
            // Get filepath
            string filePath = await OpenFileDialogAsync("Select the JSON file to load", new List<string> { "json" }, "JSON files");
            ...
            {
                await Views.MessageBox.ShowAsync((Application.Current!.ApplicationLifetime as ClassicDesktopStyleApplicationLifetime)!.MainWindow!, "Error", "Failed to load the JSON file.\nMake sure the file is valid, then try again.", Views.MessageBox.MessageButtons.Ok);
                return;
            }
        ...
        SaveCommand = ReactiveCommand.CreateFromTask(async () =>
        {
            ...
            {
                await Views.MessageBox.ShowAsync((Application.Current.ApplicationLifetime as ClassicDesktopStyleApplicationLifetime)!.MainWindow!, "Error", "Failed to save the mod JSON.\nTry saving into another folder.", Views.MessageBox.MessageButtons.Ok);
                return;
            }
